      ...
      <div className="Main w-6/12 h-auto p-4 bg-slate-950 rounded-lg min-w-[960px]">
        ...
        <div className="Header mb-4 h-[40px] flex justify-between gap-3 top-0 z-10 m-4">
          ...
          <div className="btn w-1/6 flex justify-center align-items">
            {["top", "jungle", "mid", "ad", "support"].map(
              (position, index) => (
                <button
                  key={index}
                  onClick={() => handlePositionFilter(position)}
                  className={`bg-slate-950 hover:bg-gray-400 text-gray-800 font-bold py-1 px-1 rounded-${
                    index === 0 ? "l" : index === 4 ? "r" : "c"
                  } ${
                ...
                >
                  <Image
                    src={`/images/Position_${position}.png`}
                    width={30}
                    height={30}
                    alt={`Position_${position}`}
                  />
                </button>
              )
            )}
          ...
          <div className="btn w-1/3 flex items-center justify-center">
            ...
            {quetype.map((type, index) => (
              <button
                ...
                className={`hover:text-yellow-600 text-white text-xs font-bold py-2 px-4 rounded${
                  index === 0
                    ? "-c"
                    : index === quetype.length - 1
                    ? "-r"
                    : "-c"
                } ${selectedType === type ? "bg-gray-400" : "bg-gray-800"}`}
              >
                {type}
              </button>
            ))}
          ...
          {/* 듀오 구하기 버튼 */}
          <button
            className="bg-yellow-600 hover:bg-white hover:text-yellow-600 text-white font-bold py-2 px-4 rounded"
            onClick={openModal}
          >
            듀오 구하기
          </button>
